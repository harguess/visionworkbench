...
#define UNIQUE "16x16_oldcode_olddem_jacobmatfix4"
...
Vector3 ComputeNormalDerivative(int flag,  Vector3 xyz, Vector3 xyzTOP, Vector3 xyzLEFT)
{
  ...
  // according to the document, i is row, j is column.  this means that
  // left is i,j-1 and top is i-1,j
  // this definition is now consistent with the (updated) document
...
vw::photometry::UpdateHeightMap(ModelParams inputImgParams, std::vector<ModelParams> overlapImgParams, GlobalParams globalParams) { 
  ...
  for (int kb = 0; kb < numVerBlocks; ++kb) {
    for (int lb = 0; lb < numHorBlocks; ++lb) {
      ...
      if ( n < ExtendedArea ) {//why is this using augmentedblockarea and not dummyarea?
          ...
          if ((ii < inputImage.rows()) && (jj < inputImage.cols())){
            ...
            if ( is_valid(inputImage(jj,ii)) ) {
              ...
              if ((input_img_left_pix(0) >= 0) && (input_img_top_pix(1) >= 0) && (interp_dem_image(x,y) != -10000)){
                ...
                //std::cout << "l index: " << l_index << " xyz_top: " << xyz_top << " xyz_left: " << xyz_left << " xyz_current:  " << xyzArray[l_index] << " normalArray: " << normalArray[l_index] << std::endl;
      ...
      for (k = temp; k < verExtendedSize; ++k) { 
        for (l = temp; l < horExtendedSize; ++l) { 
          ...
          if ((ii < inputImage.rows()) && (jj < inputImage.cols())){
            ...
            //printf("jj = %d, ii = %d\n", jj, ii);
              //std::cout << "shadow: " << shadowImage(jj,ii) << endl;
             // std::cout << "inputImage: " << inputImage(jj,ii) << endl;
              //std::cin.get();
            ...
            if (is_valid(inputImage(jj,ii)) && (shadowImage(jj, ii) == 0)){
              ...
              std::cout << "lb, kb: " << lb << " " << kb << " l, k: " << l << " " << k << " l_index: " << l_index << endl;
              ...
              if ( (l != horExtendedSize-1) && (k != verExtendedSize-1) ) {
                ...
                std::cout << "recDer " << recDer << " jacob: " << recDer*weight << endl;
              ...
              if((l_index >= horExtendedSize) && (l != horExtendedSize-1)) {
                ...
                std::cout << "recDerTOP " << recDerTOP << " jacob: " << recDerTOP*weight << endl;
                jacobianArray[0](l_index, l_index-horExtendedSize) = recDerTOP*weight;
                //std::cout << recDerTOP*weight << endl;
                //std::cin.get();
              }
              //if (l_index > horExtendedSize-1){
              //left
              //josh changed to not compute left when we are at the first column or the last row
              if((l != 0) && (k != verExtendedSize-1)) {
                ...
                std::cout << "recDerLEFT " << recDerLEFT << " jacob: " << recDerLEFT*weight << endl;
              ...
              //std::cout << "lb, kb: " << lb << " " << kb << " l, k: " << l << " " << k << endl;
              //std::cout << "shadow: " << shadowImage(jj,ii) << endl;
              //std::cout << "inputImage: " << inputImage(jj,ii) << endl;
              //std::cin.get();
...
//            if (l_index >= horBlockSize) finalJacobianArray[m](ll_index, ll_index-horBlockSize) = jacobianArray[m](l_index, l_index-horExtendedSize); //top
//            finalErrorVectorArray[m](ll_index) = errorVectorArray[m](l_index);
//          } // l
//        } // k 

        //experimental - finalJacobian should be the same as the final
        int oldcol = 0;
        int tmpBlockSize;
        for (int row = 0; row < AugmentedBlockArea; ++row) {
          finalErrorVectorArray[m](row) = errorVectorArray[m](row);
          for (int col = 0; col < BlockArea; ++col) {
            if (col == 0) {
              oldcol = 0;
              tmpBlockSize = horBlockSize;
            }
            if ((oldcol % tmpBlockSize == 0) && (col != 0)){
              oldcol++;
              tmpBlockSize = tmpBlockSize+horBlockSize+1;
            }
            finalJacobianArray[m](row,col) = jacobianArray[m](row,oldcol);
            //std::cout << "row: " << row << " col: " << col << " oldcol: " << oldcol << endl;
            oldcol++;
            //finalErrorVectorArray[m](row
          }
        }
...
//      //std::cin.get();
      ...
      //print the jacobian
      for (int row = 0; row< ExtendedArea; row++){
        //std::cout << row << ": ";
        for (int col = 0; col < ExtendedArea; col++){
          //if (jacobianArray[0](row, col) == 0){
          ...
          //std::cout << finalJacobianArray[0](row, col) << ","; 
        }
        f << endl;
        //std::cout << endl;
      }
      ...
      for (int row = 0; row< AugmentedBlockArea; row++){
        //std::cout << row << ": ";
        for (int col = 0; col < BlockArea; col++){
          //if (finalJacobianArray[0](row, col) == 0){
            f << finalJacobianArray[0](row,col) << ",";
            //f << 0 << ",";
            //std::cout << 0 << ",";
          //}
